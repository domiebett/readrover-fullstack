...
from app.services.book_service import (
    create_book, get_user_books, update_book,
...
@pytest.mark.asyncio
async def test_create_book_success(sample_book_data):
    ...
    result = await create_book(db, sample_book_data, user_id=1)
...
@pytest.mark.asyncio
async def test_create_book_database_error(sample_book_data):
    """Test book creation with database error."""
    db = make_mock_db_commit(commit_side_effect=IntegrityError("", "", ""))

    with pytest.raises(IntegrityError):
        await create_book(db, sample_book_data, user_id=1)
...
@pytest.mark.asyncio
async def test_get_user_books_success(sample_user_book):
    """Test getting user books."""
    db = make_mock_db_execute([sample_user_book])

    result = await get_user_books(db, user_id=1, skip=0, limit=10)
...
@pytest.mark.asyncio
async def test_update_book_success(sample_book):
    ...
    result = await update_book(db, book_id=1, book_data=update_data)
...
async def test_update_book_not_found():
    ...
    result = await update_book(db, book_id=999, book_data=update_data)

    assert result is None


@pytest.mark.asyncio
async def test_update_user_book_success(sample_user_book):
...
@pytest.mark.asyncio
async def test_update_user_book_not_found():
...
@pytest.mark.asyncio
async def test_delete_book_success(sample_user_book, sample_book):
    """Test deleting a book."""
    # Mock the service methods directly
    with pytest.MonkeyPatch().context() as m:
        m.setattr("app.services.book_service.get_user_book", AsyncMock(return_value=sample_user_book))
        m.setattr("app.services.book_service.get_book", AsyncMock(return_value=sample_book))
        m.setattr("app.services.book_service._storage_service.delete_book_files", AsyncMock())
        ...
        result = await delete_book(db, book_id=1, user_id=1)

        assert result is True
        # The storage service call is now handled by the module-level _storage_service
        db.delete.assert_called()
        db.commit.assert_awaited_once()


@pytest.mark.asyncio
async def test_delete_book_user_not_found():
...
@pytest.mark.asyncio
async def test_share_book_success(sample_user_book):
    """Test sharing a book successfully."""
    # Mock the service methods directly - first call returns sharer's book, second returns None
    with pytest.MonkeyPatch().context() as m:
        m.setattr("app.services.book_service.get_user_book", AsyncMock(side_effect=[sample_user_book, None]))
        m.setattr("app.services.book_service.get_user_by_email_case_insensitive", AsyncMock(return_value=Mock(id=2, email="recipient@example.com")))
...
@pytest.mark.asyncio
async def test_share_book_not_found():
...
@pytest.mark.asyncio
async def test_share_book_cannot_share():
...
@pytest.mark.asyncio
async def test_share_book_recipient_not_found(sample_user_book):
...
@pytest.mark.asyncio
async def test_share_book_already_shared(sample_user_book):
    """Test sharing a book that's already shared with recipient."""
    # Mock the service methods directly
    with pytest.MonkeyPatch().context() as m:
        m.setattr("app.services.book_service.get_user_book", AsyncMock(side_effect=[sample_user_book, sample_user_book]))
        m.setattr("app.services.book_service.get_user_by_email_case_insensitive", AsyncMock(return_value=Mock(id=2, email="recipient@example.com")))
...
@pytest.mark.asyncio
async def test_get_shared_books(sample_user_book):
    """Test getting books shared with a user."""
    db = make_mock_db_execute([sample_user_book])

    result = await get_shared_books(db, user_id=2)
...
@pytest.mark.asyncio
async def test_get_books_shared_by_user(sample_user_book):
    """Test getting books shared by a user."""
    db = make_mock_db_execute([sample_user_book])

    result = await get_books_shared_by_user(db, user_id=1)
