...
async def test_get_user_by_email_case_insensitive_found():
    user = User(id=1, first_name="A", last_name="B", email="test@example.com", hashed_password="password123")
    db = make_mock_db_execute(user)
    result = await user_service.get_user_by_email_case_insensitive(db, "test@example.com")
    assert result == user
...
async def test_get_user_by_email_case_insensitive_not_found():
    ...
    result = await user_service.get_user_by_email_case_insensitive(db, "notfound@example.com")
