...
class BookService:
    ...
    async def create_book(
        self, db: AsyncSession, book_data: BookCreate, user_id: int
    ) -> Book:
    ...
    async def update_book(
        self, db: AsyncSession, book_id: int, book_data: BookUpdate
    ) -> Optional[Book]:
        """Update a book."""
    ...
    async def update_user_book(
        ...
        user_book = await self.get_user_book(db, user_id, book_id)
        if not user_book:
            return None
    ...
    async def delete_book(
        ...
        if book.file_type:
            await self.storage_service.delete_book_files(
    ...
    async def share_book(
        ...
        sharer_book = await self.get_user_book(db, sharer_id, book_id)
        if not sharer_book:
            raise HTTPException(
        ...
        existing_share = await self.get_user_book(db, recipient.id, book_id)
        if existing_share:
            raise HTTPException(
